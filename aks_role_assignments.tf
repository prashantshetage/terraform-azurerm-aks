...
resource "azurerm_role_assignment" "role_assignment_node_rg" {
  for_each             = var.csi_with_aadpod_id ? var.role_assignment_node_rg : {}
  scope                = data.azurerm_resource_group.aks_node_rg.id
  principal_id         = azurerm_kubernetes_cluster.aks_cluster.kubelet_identity[0].user_assigned_identity_id
...
resource "azurerm_role_assignment" "role_assignment_others" {
  for_each             = var.csi_with_aadpod_id ? var.role_assignment_others : {}
